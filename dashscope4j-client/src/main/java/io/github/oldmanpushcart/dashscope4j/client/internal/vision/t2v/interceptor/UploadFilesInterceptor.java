package io.github.oldmanpushcart.dashscope4j.client.internal.vision.t2v.interceptor;
...
public class UploadFilesInterceptor implements TaskInterceptor {
    ...
    public CompletionStage<? extends Task.Half<?>> intercept(Chain chain) {
        ...
        if (!(chain.request() instanceof Text2VideoRequest request)) {
            return chain.proceed();
        }

        if (!request.uploadEnabled() || !isFileURI(request.audio())) {
            return chain.proceed();
        }

        return chain.client().base().store().upload(request.audio(), request.model())
                .thenCompose(uploadedURI -> {
                    final var newRequest = Text2VideoRequest.newBuilder(request)
                            .audio(uploadedURI)
                            .build();
