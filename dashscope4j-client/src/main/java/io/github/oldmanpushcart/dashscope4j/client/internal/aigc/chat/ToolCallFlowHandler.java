 ...
 * 流式工具调用处理器
 */
public class ToolCallFlowHandler implements UnaryOperator<Flow.Publisher<AigcResponse<Output>>> {
    ...
    public Flow.Publisher<AigcResponse<Output>> apply(Flow.Publisher<AigcResponse<Output>> publisher) {
        final var requestRef = new AtomicReference<AigcRequest<Input, Output, ChatModel>>();
...
//                .filter(response -> !response.output().choices().isEmpty())
                ...
                .filter(response -> {
                    ...
                    if (requestRef.get() == null) {
                        ...
                        requestRef.set((AigcRequest<Input, Output, ChatModel>) response.request());
                    }
    ...
    private AssistantMessage mergeSegments(AigcRequest<Input, Output, ChatModel> request, List<AssistantMessage> segments) {
        if (null == request) {
            return null;
        }
