...
class FunctionToolCaller implements Tool.Caller {
    ...
    private final AigcRequest<Input, Output, ChatModel> request;
    private final AssistantMessage message;

    public FunctionToolCaller(ChatOp chatOp, AigcRequest<Input, Output, ChatModel> request, AssistantMessage message) {
        this.chatOp = chatOp;
    ...
    private AigcRequest<Input, Output, ChatModel> newHistoryRequest(List<Message> history) {
    ...
    @Override
    public AigcRequest<?, ?, ChatModel> request() {
        return request;
    }
