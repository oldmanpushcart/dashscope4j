...
import static io.github.oldmanpushcart.dashscope4j.client.Exchange.NORMAL_CLOSURE;
...
class ExchangeWebSocketListenerImpl<T extends ApiRequest<R>, R extends ApiResponse<?>> extends WebSocketListener {
    ...
    ExchangeWebSocketListenerImpl(final CompletableFuture<Exchange<T>> exchangeF,
                                  ...
                                  final BiFunction<Response, String, R> decoder) {
    ...
    public void onClosed(@NotNull WebSocket socket, int code, @NotNull String reason) {
        ...
        if (NORMAL_CLOSURE == code) {
            ...
            final Exception ex = new IOException("Internal closed! code=%s;reason=%s;".formatted(
                    code,
                    reason
            ));
            listener.onError(ex);
    ...
    private record InFrame(

            @JsonProperty("header")
            ExchangeWebSocketListenerImpl.InFrame.Header header,
