package io.github.oldmanpushcart.dashscope4j.internal.base.tokenizer.local;
...
class QwenTokenizer {
    ...
    private static Map<EncodeBytesEntity, Integer> initBpeMerges() {
        final Map<EncodeBytesEntity, Integer> map = new LinkedHashMap<>();
        try (final Scanner scanner = new Scanner(Objects.requireNonNull(QwenTokenizer.class.getClassLoader().getResourceAsStream(vocabularyBpeFile)), "UTF-8")) {
            while (scanner.hasNextLine()) {
                final String line = scanner.nextLine();

                final String[] splits = line.split(TOKEN_RANK_SEPARATOR);
                assert splits.length == 2 : "Invalid line in " + vocabularyBpeFile + ": " + line;

                final byte[] token = Base64.getDecoder().decode(splits[0].getBytes(UTF_8));
                final int rank = Integer.parseInt(splits[1]);
    ...
    private static Map<Integer, byte[]> initReverseBpeMerges() {
        final Map<Integer, byte[]> map = new HashMap<>(bpeMerges.size() + specialBpeMerges.size());
    ...
    private static EncodeBytesEntity mergePair(EncodeBytesEntity first, EncodeBytesEntity second) {
        final byte[] mergeBytes = new byte[first.bytes.length + second.bytes.length];
    ...
    private static EncodeBytesEntity[] merge(EncodeBytesEntity[] ids, EncodeBytesEntity bytePair) {
        final EncodeBytesEntity[] merged = new EncodeBytesEntity[ids.length];
        int mergedIndex = 0;
        for (int i = 0; i < ids.length; ) {
            if (i < ids.length - 1) {
                final EncodeBytesEntity mergePair = mergePair(ids[i], ids[i + 1]);
    ...
    private EncodeBytesEntity getLowestIndexBytePair(EncodeBytesEntity[] ids) {
        final Set<EncodeBytesEntity> uniqueSet = new HashSet<>();
        ...
        for (int i = 0; i < ids.length - 1; ++i) {
            ...
            final Integer rank = bpeMerges.get(mergePair);
            if (rank == null) {
    ...
    private List<Integer> encodeChunk(String chunk) {
        final byte[] chunkBytes = chunk.getBytes(UTF_8);
    ...
    public List<Integer> encodeOrdinary(String text) {
        final List<Integer> tokenIds = new ArrayList<>();
        // 1. split the input text to trunks use regex
        for (final Matcher matcher = PATTEN_STRING.matcher(text); matcher.find(); ) {
            // encode the chunk.
    ...
    public List<Integer> encode(String text, SpecialTokenMode mode) {
        ...
        final Map<String, Integer> usedSpecialBpeMerges = initUsedSpecialBpeMerges(text, mode);
         ...
         * ]
         */
        final List<String> chunks = splitWithSpecial(text);
    ...
    public static void main(String... args) {
        final QwenTokenizer tokenizer = new QwenTokenizer();
