...
import io.github.oldmanpushcart.dashscope4j.agent.toolbox3.indexer.ToolIndexer;
...
public class HashMapToolbox implements Toolbox {
    ...
    private class Syncer extends Thread {
        ...
        private final List<ChangeEvent> eventQueue = new ArrayList<>();
        ...
        void notifyChange(List<ToolUse> upserts, List<String> removes, ToolSubscription subscription) {
            lock.lock();
            try {
                eventQueue.add(new ChangeEvent(upserts, removes, subscription));
                waiting.signalAll();
            } finally {
                lock.unlock();
            }
        ...
        public void run() {
            ...
            while (!isInterrupted()) {
                ...
                try {
                    ...
                    // 开始执行同步（使用迭代器，失败不删除）
                    final var eventIt = eventQueue.iterator();
