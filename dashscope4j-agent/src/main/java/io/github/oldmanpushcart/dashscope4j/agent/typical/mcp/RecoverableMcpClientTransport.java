...
public class RecoverableMcpClientTransport implements McpClientTransport {
    ...
    private RecoverableMcpClientTransport(Builder builder) {
        ...
        this.mapper = requireNonNullElseGet(builder.mapper, ObjectMapper::new);
        this.transportFactory = requireNonNull(builder.transportFactory);
    ...
    public Mono<Void> closeGracefully() {
        ...
        final var closingF = CompletableFuture.completedStage(null)
                ...
                .exceptionally(ex -> {
                    log.debug("{} closing holder failed.", this, ex);
                    return null;
                })
                ...
                .exceptionally(ex -> {
                    log.debug("{} closing scheduler failed.", this, ex);
                    return null;
                })

                // finally closed
                .thenAccept(unused -> log.debug("{} closed.", this));
    ...
    public interface McpClientTransportFactory {
         ...
         */
        McpClientTransport create(ObjectMapper mapper);
        ...
        private Future<?> schedulingPingTimeout(String pingId, CompletableFuture<Void> pongF) {
            return scheduling(() -> {
                if (pongF.cancel(true)) {
                    log.debug("{}/heartbeat/{} timeout!", RecoverableMcpClientTransport.this, pingId);
                }
            }, pingInterval);
        }
